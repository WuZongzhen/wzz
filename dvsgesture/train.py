...
import pdb
from load_dataset import dataset_video
...
def train_one_epoch(model, criterion, optimizer, data_loader, device, epoch, print_freq, scaler=None, T_train=None):
    ...
    for image, target in metric_logger.log_every(data_loader, print_freq, header):
        ...
        if T_train:
            ...
            sec_list = np.random.choice(image.shape[1], T_train, replace=True)
            pdb.set_trace()
            sec_list.sort()
...
def load_data(traindir, valdir, distributed):
    # Data loading code
    print("Loading data")
...
def main(args):
    ...
    dataset_train, dataset_test, train_sampler, test_sampler = load_data(train_dir, val_dir, args.distributed)
